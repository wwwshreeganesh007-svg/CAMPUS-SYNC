...
app = Flask(__name__)
app.secret_key = "xime_campussync_secret_key"
...
def init_db():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS users (
        email TEXT PRIMARY KEY, name TEXT, password TEXT, role TEXT, club TEXT, status TEXT
    )''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS clubs (
        id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT UNIQUE, category TEXT, description TEXT, coordinator TEXT, threshold INTEGER DEFAULT 75
    )''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS events (
        id INTEGER PRIMARY KEY AUTOINCREMENT, club TEXT, name TEXT, date TEXT, start_time TEXT, end_time TEXT, venue TEXT, code TEXT, status TEXT DEFAULT 'Active', expires_at TEXT
    )''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS members (
        email TEXT, name TEXT, club TEXT, attendance_pct INTEGER DEFAULT 100, PRIMARY KEY (email, club)
    )''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS attendance (
        id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT, event TEXT, status TEXT, reason TEXT
    )''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS correction_requests (
        id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT, club TEXT, event TEXT, reason TEXT, status TEXT DEFAULT 'Pending'
    )''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS audit_logs (
        id INTEGER PRIMARY KEY AUTOINCREMENT, action TEXT, user TEXT, details TEXT, timestamp TEXT
    )''')
    
    # Seed default accounts and clubs for the demo
...
def club_admin():
    ...
    attendance_data = conn.execute('''SELECT * FROM attendance WHERE event IN (SELECT name FROM events WHERE club = ?)''', (my_club['name'],)).fetchall()
    conn.close()
...
def student_dashboard():
        ...
        if action == 'mark_attendance':
            ...
            if ev:
                ...
                if datetime.now() > exp_dt:
                    flash("❌ Session Code Expired (15-min TTL exceeded).", "error")
                else:
                ...
                flash("❌ Invalid or Cancelled Code", "error")
    ...
    student_stats = {
        'total_clubs': 1, 'workshops_attended': attended_count, 'avg_attendance': current_pct,
